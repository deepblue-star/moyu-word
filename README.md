...
- 运行 `MoyuWord-Setup-x86.exe` 安装到当前用户；或解压 `MoyuWord-x86.zip`，运行 `MoyuWord.exe`。便携版必须保留同目录的 `assets` 和 `native` 文件夹。
...
词库导入、收藏、透明度及窗口位置保存到 `%LOCALAPPDATA%\MoyuWord`。卸载保留这些学习数据。运行时没有网络请求、账号、云同步、更新检查或遥测。开发阶段下载测试词库和 PDF 引擎的脚本不包含在运行逻辑中。
